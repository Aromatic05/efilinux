...
set +e
{
    sleep 75
    printf '%s\n' \
        "test \"\$(cat /proc/1/comm)\" = init && /usr/bin/printf 'EFILINUX_PID1=init\\n' && /usr/bin/udevadm control --ping && for service in syslogd dbus crond elogind polkit upower iwd networkmanager udisks2 sshd; do test -s /run/efilinux/\$service.pid && kill -0 \"\$(cat /run/efilinux/\$service.pid)\" || exit 1; done && test ! -e /run/efilinux/dhcpcd.pid && /usr/bin/printf 'EFILINUX_SERVICES_OK\\n' && /usr/bin/modprobe --version && /usr/bin/modprobe loop && /usr/bin/lsmod && /usr/bin/printf 'EFILINUX_MODULE_LOAD_OK\\n' && /usr/bin/poweroff -f"
