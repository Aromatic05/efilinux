...
set +e
guest_command=$(cat <<'GUEST'
set -e
test "$(cat /proc/1/comm)" = init
/usr/bin/printf 'EFILINUX_PID1=init\n'
/usr/bin/udevadm control --ping
for service in syslogd dbus crond elogind polkit upower iwd networkmanager udisks2 sshd; do
    test -s "/run/efilinux/$service.pid"
    kill -0 "$(cat "/run/efilinux/$service.pid")"
done
test ! -e /run/efilinux/dhcpcd.pid
/usr/bin/printf 'EFILINUX_SERVICES_OK\n'
/usr/bin/modprobe --version
/usr/bin/modprobe loop
/usr/bin/lsmod
/usr/bin/printf 'EFILINUX_MODULE_LOAD_OK\n'
test "$(/usr/bin/awk -F: '$1 == "root" { print $7 }' /etc/passwd)" = /usr/bin/bash
/usr/bin/bash -c 'set -o pipefail; values=(alpha beta); [[ ${values[1]} == beta ]]'
/usr/bin/stat -c %s /usr/bin/bash >/dev/null
test "$(/usr/bin/readlink -f /usr/bin/sh)" = /usr/bin/busybox
/usr/bin/find /usr -mindepth 1 -maxdepth 1 -print0 >/dev/null
/usr/bin/printf 'EFILINUX_GNU_RUNTIME_OK\n'
/usr/bin/poweroff -f
GUEST
)
{
...
if ! grep -q 'EFILINUX_GNU_RUNTIME_OK' "$boot_log"; then
    tail -n 120 "$boot_log" >&2
    die "Bash or GNU runtime commands failed inside the booted system"
fi

log "OVMF boot reached SysVinit multi-user mode with services, Kmod, and GNU runtime operational"
printf 'Boot log: %s\n' "$boot_log"
