#!/usr/bin/env bash
set -euo pipefail
ROOT=$(cd -- "$(dirname -- "$0")/../.." && pwd)
source "$ROOT/config.sh"
source "$ROOT/lib/common.sh"
source "$ROOT/lib/package.sh"
require_command autoreconf curl gcc make pkg-config sha256sum tar
ensure_directories
package="ntfs-3g-$NTFS_3G_VERSION"
if binary_package_restore_sysroot "$package" "${BASH_SOURCE[0]}"; then
    exit 0
fi
archive="$EFILINUX_DOWNLOADS/$package.tar.gz"
prepare_package "$package"
download "https://github.com/tuxera/ntfs-3g/archive/refs/tags/$NTFS_3G_VERSION.tar.gz" "$archive"
verify_sha256 "$NTFS_3G_SHA256" "$archive"
extract_source "$archive" "$PACKAGE_SOURCE"
log "Preparing NTFS maintenance tools"
autoreconf -fi "$PACKAGE_SOURCE"
log "Configuring NTFS maintenance tools"
cd "$PACKAGE_BUILD"
CC=gcc CFLAGS="$(target_cflags)" LDFLAGS="$(target_ldflags)" \
PKG_CONFIG_SYSROOT_DIR="$EFILINUX_SYSROOT" \
PKG_CONFIG_LIBDIR="$EFILINUX_SYSROOT/usr/lib/pkgconfig:$EFILINUX_SYSROOT/usr/share/pkgconfig" \
    "$PACKAGE_SOURCE/configure" --prefix=/usr --bindir=/usr/bin --sbindir=/usr/bin \
        --libdir=/usr/lib --disable-static --disable-ntfs-3g --disable-plugins \
        --enable-extras --disable-nls
log "Building NTFS maintenance tools"
make -j"$EFILINUX_JOBS"
make DESTDIR="$PACKAGE_STAGING" install
find "$PACKAGE_STAGING/usr/lib" -maxdepth 1 \( -name '*.a' -o -name '*.la' \) -delete 2>/dev/null || true
binary_package_publish_sysroot "$package" "${BASH_SOURCE[0]}"
